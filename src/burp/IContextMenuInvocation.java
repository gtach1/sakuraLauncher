...
public interface IContextMenuInvocation {
     ...
     * panel, then the <code>IHttpRequestResponse</code> returned by this method
     * will reflect the current contents of the interception panel, and this
     * will change when the current message has been forwarded or dropped. If
     * your extension needs to store details of the message for which the
     * context menu has been invoked, then you should query those details from
     * the <code>IHttpRequestResponse</code> at the time of invocation, or you
     * should use <code>IBurpExtenderCallbacks.saveBuffersToTempFiles()</code>
     * to create a persistent read-only copy of the
